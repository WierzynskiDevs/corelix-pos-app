...
export default function ProdutoNovoScreen() {
  ...
  const salvar = async () => {
    ...
    const vCompra = parseFloat(valorCompra.replace(",", ".")) || 0;
    const qtd = Math.max(0, parseInt(quantidadeEstoque, 10) || 0);
    setSaving(true);
    try {
  ...
  return (
    <ThemedView style={styles.container}>
      <ScrollView contentContainerStyle={styles.scroll}>
        ...
        <ThemedText style={styles.label}>Valor de compra (R$) – não editável depois</ThemedText>
        <TextInput style={styles.input} value={valorCompra} onChangeText={setValorCompra} placeholder="0,00" keyboardType="decimal-pad" />
        ...
        {v > 0 && (
          <ThemedText style={styles.valorFinal}>Valor final: R$ {valorF.toFixed(2)}</ThemedText>
        )}
        <ThemedText style={styles.label}>Fornecedor</ThemedText>
        <View style={styles.row}>
          <TouchableOpacity
            style={[styles.chip, !fornecedorId && styles.chipActive]}
          ...
          {fornecedores.map((f) => (
            <TouchableOpacity
              key={f.id}
              style={[styles.chip, fornecedorId === f.id && styles.chipActive]}
